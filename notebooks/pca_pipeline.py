# Script input: a given grid_id, n_components and dataset
# Script output (under directory "temp"): pca_df.csv (dataframe of points with PC1, PC2, labels), 
# normalized_input.csv (dataframe of points after normalization), pca_df.jpg (plot of pca_df)
# Refer to `pca_martin.ipynb` to see how to use this preprocessing script 

######################## TO EDIT: Select hyperparameters here ########################
# choose between 333519, 333346
grid_id = 333346
# grid_id = 333519


# number of PCA components (in this case, reduce to 2 dimensions)
n_components = 2

# Data to perform PCA on
import pandas as pd
data_heatmap_train = pd.read_csv('../datasets/train/data_heatmap_train.csv')
...
data_heatmap_expanded = data_heatmap_train.loc[data_heatmap_train.index.repeat(data_heatmap_train['clicks'])].reset_index(drop=True)
...
max_width = data_heatmap_train['display_width'].max()
max_height = data_heatmap_train['display_height'].max()
...
domains_grids = data_heatmap_train[['domain', 'grid_id']].drop_duplicates()
